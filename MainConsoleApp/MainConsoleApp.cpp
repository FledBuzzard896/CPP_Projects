...
int main()
{
    ...
    // Обнуление структур в памяти
    ZeroMemory(&si, sizeof(si));
    si.cb = sizeof(si); // указываем размер структуры
    ZeroMemory(&pi, sizeof(pi));

    wchar_t cmd[] = L"notepad.exe C:\\Users\\234885\\Desktop\\cfsd.txt";

    if (!CreateProcess(
        NULL,                // Путь к файлу
        cmd,                 // Командная строка (аргументы)
        NULL,                // Атрибуты защиты процесса
        NULL,                // Атрибуты защиты потока
        FALSE,               // Наследование дескрипторов
        0,                   // Флаги создания (напр. CREATE_NEW_CONSOLE)
        NULL,                // Среда окружения
        NULL,                // Текущий каталог
        &si,                 // Информация о запуске
        &pi                  // Информация о созданном процессе (выходные данные)
    )) 
    {
        cout << "Ошибка запуска: " << GetLastError() << endl;
        return 1;
    }

    cout << "Процесс запущен. ID: " << pi.dwProcessId << endl;
    WaitForSingleObject(pi.hProcess, INFINITE);
    cout << "Блокнот закрыт." << endl;

    CloseHandle(pi.hProcess);
    CloseHandle(pi.hThread);

    // Передача аргументов
    // Как раз таки что использовалось в примере выше:
    // wchar_t cmd[] = L"notepad.exe C:\\my_folder\\test.txt";

    // CreateProcess(
    //     NULL, // Путь берем из командной строки
    //     cmd,  // Командная строка (изменяемый буфер!)
    //     // ... остальные параметры ...
    // );

    // Скрытие окна процесса
    // За это отвечает структура STARTUPINFO. Чтобы это сработало, нужно:
    // 1.	Установить флаг STARTF_USESHOWWINDOW.
    // 2.	В поле wShowWindow указать SW_HIDE.
    ...
    // Завершение процесса
    // Если вам нужно принудительно закрыть созданный процесс из своей программы,
    // используйте дескриптор pi.hProcess
    // 
    // 0 — это код выхода, который получит система
...
//
//    InitAudioDevice();
//    Sound testsound = LoadSound(
//        "./media/testsound1.wav"
//    );
//
//    PlaySound(testsound);
